...
class Video:  #Handles basic video functions
    ...
    def createNewWindow(self, name, **kwargs):
        ...
        xPos = kwargs.get("xPos", 800)
        yPos = kwargs.get("yPos", 800)
...
class ObjectTracker:
    ...
    def __init__(self, video, **kwargs):
        ...
        trackingPoints = kwargs.get("pointsToTrack", 800)         #Number of keypoints to find per frame
    ...
    def drawEdged(self, **kwargs):
        ...
        gray = cv2.cvtColor(frameToAnalyze, cv2.COLOR_GRAY2RGB)
    ...
    def drawShapes(self, shapeTargets, **kwargs):  #RETURNS A FRAME WITH THE SHAPES DRAWN ON IT
        ...
        for shapeTarget in shapeTargets:
            if hasattr(shapeTarget, 'center'):
                cv2.circle(frameToDraw, (tuple(map(int, shapeTarget.center))), 10, (0, 0, 255), 1)
...
==
    ...
    def getMovement(self, **kwargs):
        ...
        if len(self.vid.previousFrames) > 10:
